...
项目在 [vite.config.ts](/vite.config.ts) 中配置了以下内容：
...
项目使用 TypeScript 提供类型安全，配置文件为 [tsconfig.json](/tsconfig.json)：
...
项目使用 Tailwind CSS 作为主要的样式系统，配置文件为 [tailwind.config.js](/tailwind.config.js)：
...
项目使用 PostCSS 处理 CSS，配置文件为 [postcss.config.js](/postcss.config.js)：
...
项目在 [package.json](/package.json) 中定义了以下构建相关脚本：
