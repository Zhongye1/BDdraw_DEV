...
- **代码对应**：
  ...
  在 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 文件中，这个操作是变换流程的第一步，用于将世界坐标系中的点转换到组的局部坐标系中。
...
- **代码对应**：
  ...
  这部分代码在 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 中实现，通过计算点在原始组局部坐标系中的相对位置，然后映射到新的组局部坐标系中。
...
- **代码对应**：
  ...
  这是 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 中变换流程的最后一步，将变换后的局部坐标点重新转换回世界坐标系。
...
这部分实现在 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 的第 394-420 行左右。
...
在代码中，这个逻辑实现在 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 和 [TF_controler_Renderer.ts](/src/pages/canvas/Pixi_STM_modules/core/TF_controler_Renderer.ts) 中，通过检查所有选中元素的旋转角度是否一致来决定组的角度：
...
这个完整的流程在 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 的 `onPointerMove` 方法中实现，特别是在处理 `resizing` 模式时。
...
这个逻辑在 [TF_controler_Renderer.ts](/src/pages/canvas/Pixi_STM_modules/core/TF_controler_Renderer.ts) 的 `renderTransformer` 方法中实现。
...
在 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 中，这部分逻辑通过以下代码实现：
...
这个计算过程在 [TF_controler_Renderer.ts](/src/pages/canvas/Pixi_STM_modules/core/TF_controler_Renderer.ts) 中实现：
...
代码实现在 [Stage_InteractionHandler.ts](/src/pages/canvas/Pixi_STM_modules/interaction/Stage_InteractionHandler.ts) 和 [TF_controler_Renderer.ts](/src/pages/canvas/Pixi_STM_modules/core/TF_controler_Renderer.ts) 中，通过精确的数学计算和矩阵变换模拟，确保了高质量的用户体验。